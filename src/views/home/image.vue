    ...
    <GridLayout row="3" rows="*">
      <Pager row="1" col="0" colSpan="2" :selectedIndex="tab" @selectedIndexChange="onTabChange">
        <PagerItem v-for="i in 5">
          <GridLayout rows="*">
            <imageList row="0" :data="listData[i - 1]" :loading="isLoading[i - 1]" @loadMoreItems="nextPage(i - 1)"
...
function getList(tab: number): Promise<ImageItem[] | null> {
  let sort: string
  switch (tab) {
  ...
  return new Promise((resolve, reject) => {
    if (!isLoading.value[tab]) {
      ...
      getImageList(page[tab], sort, filterYear.value, filterMonth.value).then(res => {
        page[tab]++
        resolve(res)
      }).catch(() => {
        reject()
